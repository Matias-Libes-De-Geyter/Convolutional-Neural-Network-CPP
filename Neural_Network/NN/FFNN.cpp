...
// ======== NEURAL NETWORK ======== //
FFNN::FFNN(const hyperparameters& hyper, int input_size) : _hyper(hyper) {
...
Matrix FFNN::backpropagation(Matrix& input, const Matrix& y_real) {
	...
	return (m_dZ[0] * m_layers[0].weights().T_then_removeBias()).hadamard(ACTIVATION::deriv_ReLU(input));
}
